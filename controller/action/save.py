...
class SaveActionController(BaseController):
    ...
    @view_config(route_name='save_action', permission='use', renderer='controller.army:templates/selection.mako')
    def save_action(self):
        ...
        if action.step == 3:
            ...
            action.save()
        return self.step_dispatcher(action, army, damage)

    def step_dispatcher(self, action, army, damage):
        if action.step == 1:
            #resolve SAI
            ...
            #save finished
            url = self.request.route_url('save_action_step_three', id=army.id, damage=damage)
            return_value = HTTPFound(location=url)
        else:
            #error !
            pass
    ...
    @view_config(route_name='save_action_step_three', permission='use', renderer='controller.action:templates/save/step_three.mako')
    def step_three(self):
        army = Army.get_by_id(int(self.request.matchdict.get('id')))
        damage = int(self.request.matchdict.get('damage'))
        army.set_roll_type(SaveMeleeRoll())
        #What are the result ?
        results = []
        for dice in army.components:
        ...
        if damage > army.get_save_result():
            remaining = damage - army.get_save_result()
        else:
            remaining = 0
        return {'results': results, 'dices': dice_list, 'save': army.get_save_result(), 'damage': damage, 'remaining': remaining, 'route_next_step': self.request.route_url('army_selection')}
