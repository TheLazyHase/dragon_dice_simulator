...
class Trample(SAI, Face):
    allowed_icon = [Face.ICON_MANEUVER, Face.ICON_MELEE]
    #If a combined roll that use both maneuver and save for the same effect is rolled, trample is considered save
    #It's not a meaningful decision anyway
    main_icon = Face.ICON_MANEUVER

    @property
    def name(self):
        return '%s Trample' % self.amount

    def icon_by_type(self, icon_type):
        value = 0

        #What is the intersection between trample icon and required icon ?
        type_intersection = list(set(Trample.allowed_icon) & set(self.type_roll.allowed_icon))
        ...
        #Combined roll that allow for save, maneuver, or both
            ...
            #Combined roll that allow for neither save nor maneuver
                ...
                #Normally we should not need to know what Trample is, but in case of racial swap, it's both :
                # @warning : it will break if you somehow convert both save and maneuver to something else on the same dice
                if (icon_type in Trample.allowed_icon):
                    value = self.amount
                else:
                    value = 0
        #If the roll is not combined, we do not need human input
            ...
            if len(type_intersection) == 0:
                if (icon_type in Trample.allowed_face):
                    value = self.amount
                else:
                    value = 0
            #Count as exactly one of the needed icon
                ...
                if (icon_type in type_intersection):
                    ...
                    value = 0
            #Count as at least two of the needed icon ; considered as Trample.main_icon and nothing else
            else:
                if (icon_type == Trample.main_icon):
                    value = self.amount
                else:
                    value = 0
        else:
            #If the roll is about melee or maneuver, then trample is of the adaptated type
            if (self.type_roll.main_icon == Face.ICON_MELEE or self.type_roll.main_icon == Face.ICON_MANEUVER):
                if (icon_type == self.type_roll.main_icon):
                    value = self.amount
                else:
                    #Since it's the main roll type, it's not anything else
                    value = 0
            #Else, normally we should not need to know what Trample is, but in case of racial swap, it's both :
            # @warning : it will break if you somehow convert both melee and maneuver to something else on the same dice
            else:
                if (icon_type in Trample.allowed_icon):
                    value = self.amount
                else:
    ...
    def special_effect(self):
        ...
        #Calculate how many icon from trample are needed on this roll
